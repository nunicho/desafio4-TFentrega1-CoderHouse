...
router.post("/products/", async (req, res) => {
  ...
  if (productsToAdd.length > 0) {
    ...
    if (allProductsExist) {
      for (const product of productsToAdd) {
        const { id, quantity } = product;
        const existingCarritoIndex = carritos.findIndex((carrito) =>
          carrito.products.some((p) => p.id === id)
        );

        if (existingCarritoIndex !== -1) {
          const existingProductIndex = carritos[
            existingCarritoIndex
          ].products.findIndex((p) => p.id === id);
          if (existingProductIndex !== -1) {
            carritos[existingCarritoIndex].products[
              existingProductIndex
            ].quantity += quantity;
          } else {
            carritos[existingCarritoIndex].products.push({ id, quantity });
          }
        } else {
          const nuevoCarrito = {
            id: carritos.length > 0 ? carritos[carritos.length - 1].id + 1 : 1,
      ...
      try {
        ...
        res.status(201).json(carritos); // Agregar respuesta en caso de éxito
      } catch (error) {
        res.status(500).json({ error: "Error al guardar el carrito" });
      }
    } else {
